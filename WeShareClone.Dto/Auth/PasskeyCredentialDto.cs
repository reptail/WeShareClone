...
{
    /// <summary>The database identifier of the credential (use for deletion/renaming).</summary>
    public int Id { get; init; }

    /// <summary>The authenticator AAGUID, identifying the type of authenticator device.</summary>
    public Guid AaGuid { get; init; }

    /// <summary>The user-defined name for this passkey (e.g. "MacBook Touch ID").</summary>
    ...
    public PasskeyCredentialDto(int id, Guid aaGuid, string? name, DateTime createdAtUtc)
    {
        Id           = id;
        AaGuid       = aaGuid;
        Name         = name;
